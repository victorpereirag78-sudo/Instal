...
function filtrarModulosPorRol(rol, panelIdActual = null) {
    ...
    if (primerBotonVisible) {
        document.querySelectorAll('#main-nav [data-module]').forEach(b => b.classList.remove('active'));
...
function mostrarPanel(panelId) {
    ...
    if (moduloRequerido) {
        ...
        const moduloActual = document.querySelector('#main-nav button.active')?.dataset.module;
const moduloVisual = moduloActual || moduloPorPanel[panelId];
        document.querySelectorAll('#main-nav button').forEach(b => {
            b.classList.remove('active');
            if (b.dataset.module === moduloVisual) b.classList.add('active');
        });

        // Activar el submenu correcto en la barra lateral (excepto ingreso que no tiene submenu)
        document.querySelectorAll('#sidebar .submenu').forEach(s => s.classList.remove('active'));
        if (moduloVisual && moduloVisual !== 'ingreso') {
            const submenuActivo = document.getElementById(`submenu-${moduloVisual}`);
            if (submenuActivo) submenuActivo.classList.add('active');
        }
    }
    ...
    switch (panelId) {
        ...
        case 'panel-agendadas':
            ...
            if (regionFiltroEl) {
                populateSelect(regionFiltroEl,
                    Object.keys(appData.regiones).map(num => ({ value: num, text: appData.regiones[num].nombre })),
                    'Todas las Regiones'
                );
                regionFiltroEl.onchange = function() {
                    const comunaEl = document.getElementById('filtro-comuna');
                    if (comunaEl) {
                        const regionNum = this.value;
                        const comunas = appData.regiones[regionNum]?.comunas || [];
                        populateSelect(comunaEl, comunas.map(c => ({ value: c, text: c })), 'Seleccione Comuna');
                    }
                    aplicarFiltros();
                };
            }
        ...
        if (panelId.startsWith('panel-panol-')) {
            ...
            if (cat === 'existencia') {
                if (window.PanolModule) window.PanolModule.inicializarPanelExistencia();
            } else {
                if (window.PanolModule) window.PanolModule.renderPañol(cat);
            }
        }
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        document.querySelectorAll('#sidebar button[data-panel]').forEach(button => {
            button.addEventListener('click', () => {
                ...
                if (panelId.startsWith('panel-panol-')) {
                    const cat = panelId.replace('panel-panol-', '');
                    if (cat === 'existencia') {
                        // Panel especial: cargar técnicos en el select
                        if (window.PanolModule) window.PanolModule.inicializarPanelExistencia();
                    } else {
                        if (window.PanolModule) window.PanolModule.renderPañol(cat);
                    }
                ...
                // === FLOTA: renderizar vista al navegar ===
                if (panelId.startsWith('panel-flota-')) {
                    const view = panelId.replace('panel-flota-', '');
        ...
        // 1. Asignar fecha actual automáticamente al campo oculto (o variable)
        const fechaHoy = new Date().toISOString().split('T')[0];
        const inputFecha = document.getElementById('orden-fecha');
        if (inputFecha) {
            inputFecha.value = fechaHoy;
            inputFecha.parentElement.style.display = 'none'; // Ocultamos el contenedor de fecha
        }

        // 2. Llenar el select de Cable (0.5 en 0.5 hasta 50 metros por ejemplo)
        const selectCable = document.getElementById('orden-cable');
        if (selectCable) {
            for (let i = 0.5; i <= 50; i += 0.5) {
                const opt = document.createElement('option');
                opt.value = i;
                opt.textContent = `${i} mts`;
                selectCable.appendChild(opt);
            }
        }
    });

    // 3. Lógica de Equipos Dinámicos
    document.getElementById('orden-sub')?.addEventListener('change', function() {
        const subservicio = this.value;
        if (!subservicio) return;

        // Extraer cantidades de decos y extensores del texto del subservicio [cite: 80]
        ...
        // Obtener las opciones que ya vienen cargadas en el Modem (asumiendo que se cargan de BD) [cite: 82]
        ...
        // --- Renderizar DECOS --- [cite: 84, 87]
        const contenedorDecos = document.getElementById('contenedor-decos-ingreso');
        const labelDecos = document.getElementById('label-decos-titulo');
        if (contenedorDecos) {
            contenedorDecos.innerHTML = '';
            labelDecos.style.display = cantDecos > 0 ? 'block' : 'none';
            
            for (let i = 1; i <= cantDecos; i++) {
                const select = document.createElement('select');
                select.id = `deco-ingreso-${i}`;
                select.required = true;
                select.style.cssText = 'width:100%; padding:9px 10px; border:1px solid #d1d5db; border-radius:6px; font-size:14px; margin-bottom:5px;';
                select.innerHTML = opcionesHTML;
                if(select.options[0]) select.options[0].text = `Deco ${i} (Serie)`;
                contenedorDecos.appendChild(select);
            }
        }

        // --- Renderizar EXTENSORES --- [cite: 90, 93]
        const contenedorExts = document.getElementById('contenedor-extensores-ingreso');
        const labelExts = document.getElementById('label-extensores-titulo');
        if (contenedorExts) {
            contenedorExts.innerHTML = '';
            labelExts.style.display = cantExts > 0 ? 'block' : 'none';
            ...
            for (let i = 1; i <= cantExts; i++) {
                ...
                select.style.cssText = 'width:100%; padding:9px 10px; border:1px solid #d1d5db; border-radius:6px; font-size:14px; margin-bottom:5px;';
                select.innerHTML = opcionesHTML;
                if(select.options[0]) select.options[0].text = `Extensor ${i} (Serie)`;
...
// =======================================================
// === FUNCIONES GLOBALES PARA MODALES (FUERA DEL DOMContentLoaded) ===
// =======================================================

/**
 * Abre el modal para agregar un nuevo vehículo a la flota
 */
function abrirModalFlota() {
    const patente = prompt("Patente del vehículo:");
    if (!patente) return;
    
    const marca = prompt("Marca (ej: Toyota):") || "Toyota";
    const modelo = prompt("Modelo (ej: Hilux):") || "Hilux";
    const km = prompt("Kilometraje actual:") || "0";
    
    guardarFlota({ 
        patente, 
        marca, 
        modelo, 
        km_actual: parseInt(km), 
        km_prox_revision: parseInt(km) + 5000, 
        estado: 'Activo', 
        ultima_actualizacion: new Date().toLocaleDateString() 
    });
}

// [abrirModalPañol - consolidada en panol-module.js]
...
window.abrirModalFlota = abrirModalFlota;
...
// =======================================================
// === MÓDULO PAÑOL - OBJETO GLOBAL ===
// =======================================================
// =======================================================
// ╔═══════════════════════════════════════════════════╗
// ║           MÓDULO PAÑOL - VERSIÓN CORREGIDA        ║
// ║  EPP | Herramientas | Uniformes                   ║
// ║  Correcciones: query filtro, confirmarAsignacion, ║
// ║  stock atómico, tablas alineadas, sin duplicados  ║
// ╚═══════════════════════════════════════════════════╝
// =======================================================
...
const PanolModule = {
    ...
    // -------------------------------------------------------
    // 1. CARGAR INVENTARIO (query corregida - filtro por categoria_id)
    // -------------------------------------------------------
    async cargarInventario(tipo) {
    try {
        // PASO 1: Obtener el ID de la categoría por tipo (herramienta|epp|uniforme)
        const { data: catData, error: catError } = await supabase
            .from('panol_categorias')
            .select('id, nombre, requiere_talla')
            .eq('tipo', tipo)
            .single();

        if (catError || !catData) {
            console.warn(`⚠️ Categoría "${tipo}" no encontrada en panol_categorias`);
            return [];
        }

        // PASO 2: Filtrar items por categoria_id (evita el bug del join)
        const { data, error } = await supabase
            .from('panol_items')
            .select('*, categoria:panol_categorias(nombre, tipo, requiere_talla)')
        ...
        console.error('❌ Error cargando inventario Pañol:', err);
        return [];
    }
},

    // -------------------------------------------------------
    // 2. RENDER TABLA (columnas alineadas con el HTML)
    // -------------------------------------------------------
    renderTabla(categoria, datos) {
        const tbody = document.getElementById(`tabla-panol-${categoria}`);
        if (!tbody) {
            console.error(`❌ Elemento tabla-panol-${categoria} no encontrado`);
            return;
        }

        if (!datos || !datos.length) {
            tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;color:#888;padding:30px;">
                Sin registros en inventario. Usa "Agregar" para ingresar artículos.
            </td></tr>`;
            return;
        }

        const esUniforme = categoria === 'uniformes';
        const esEpp = categoria === 'epp';
        const esTieneVencimiento = esEpp;

        tbody.innerHTML = datos.map(item => {
            const stockDisp = item.stock_disponible ?? (item.stock_total - (item.stock_asignado || 0));
            const stockClase = stockDisp <= (item.umbral_reposicion || 5) ? 'color:#dc3545;font-weight:700;' : 'color:#198754;font-weight:600;';
            const nombreDisplay = (esUniforme || esEpp) && item.talla_modelo
                ? `${item.nombre} <span style="color:#6c757d;font-size:12px;">(${item.talla_modelo})</span>`
                : item.nombre;

            // Alerta de vencimiento
            let alertaVenc = '';
            if (esTieneVencimiento && item.fecha_vencimiento) {
                const hoy = new Date();
                const venc = new Date(item.fecha_vencimiento);
                const dias = Math.ceil((venc - hoy) / (1000 * 60 * 60 * 24));
                if (dias < 0) {
                    alertaVenc = `<span style="color:#dc3545;font-size:11px;display:block;">⚠️ VENCIDO</span>`;
                } else if (dias <= 30) {
                    alertaVenc = `<span style="color:#fd7e14;font-size:11px;display:block;">⏰ Vence en ${dias}d</span>`;
                }
            }

            // Columna de talla (solo EPP y uniformes)
            const celdaTalla = (esUniforme || esEpp)
                ? `<td style="padding:10px 12px;"><span style="background:#e9ecef;padding:2px 8px;border-radius:4px;font-size:12px;">${item.talla_modelo || '—'}</span></td>`
                : '';

            const estadoColor = item.estado === 'Disponible' ? '#198754' : item.estado === 'Agotado' ? '#dc3545' : '#6c757d';

            return `<tr style="border-bottom:1px solid #f0f0f0;">
                <td style="padding:10px 12px;font-family:monospace;color:#0056b3;font-weight:600;font-size:13px;">${item.codigo_serie || '—'}</td>
                <td style="padding:10px 12px;">${nombreDisplay}${alertaVenc}</td>
                ${celdaTalla}
                <td style="padding:10px 12px;">
                    <span style="${stockClase}">${stockDisp}</span>
                    <span style="color:#aaa;font-size:12px;"> / ${item.stock_total}</span>
                    ${stockDisp <= (item.umbral_reposicion || 5) ? '<span style="color:#dc3545;font-size:11px;display:block;">⚠️ Stock bajo</span>' : ''}
                </td>
                <td style="padding:10px 12px;"><span style="color:${estadoColor};font-weight:500;">${item.estado || 'Disponible'}</span></td>
                <td style="padding:10px 12px;display:flex;gap:6px;align-items:center;flex-wrap:wrap;">
                    <button onclick="window.PanolModule.abrirModalAsignacion('${item.id}', '${(item.nombre || '').replace(/'/g, "\'")}')"
                        style="padding:5px 12px;background:${stockDisp > 0 ? '#007bff' : '#6c757d'};color:white;border:none;border-radius:5px;cursor:${stockDisp > 0 ? 'pointer' : 'default'};font-size:13px;"
                        ${stockDisp <= 0 ? 'disabled' : ''}>
                        ${stockDisp > 0 ? '📤 Asignar' : '🔒 Agotado'}
                    </button>
                    <button onclick="window.PanolModule.abrirAjusteStock('${item.id}', '${(item.nombre || '').replace(/'/g, "\'")}', ${item.stock_total})"
                        title="Ajustar stock total"
                        style="padding:5px 10px;background:#f0f0f0;color:#555;border:1px solid #ccc;border-radius:5px;cursor:pointer;font-size:12px;">
                        ✏️
                    </button>
                </td>
            </tr>`;
    ...
    // -------------------------------------------------------
    // 3. RENDER PAÑOL (función principal de carga + render)
    // -------------------------------------------------------
    async renderPañol(categoria) {
        const tipoMap = { 'herramientas': 'herramienta', 'epp': 'epp', 'uniformes': 'uniforme' };
        const tipo = tipoMap[categoria];

        if (!tipo) {
            console.error(`❌ Categoría de pañol no válida: ${categoria}`);
            return;
        }

        const tbody = document.getElementById(`tabla-panol-${categoria}`);
        if (!tbody) {
            console.error(`❌ No se encontró tabla-panol-${categoria} en el DOM`);
            return;
        }

        tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;padding:30px;color:#888;">Cargando...</td></tr>`;
        mostrarLoader(`Cargando ${categoria}...`);

        try {
            const datos = await this.cargarInventario(tipo);
            this.renderTabla(categoria, datos);
        } catch (err) {
            console.error(`❌ Error al cargar pañol (${categoria}):`, err);
            mostrarToast(`Error al cargar ${categoria}: ${err.message}`, 'error');
        } finally {
            ocultarLoader();
        }
    },
    ...
    // -------------------------------------------------------
    // 4. ABRIR MODAL ASIGNACIÓN
    // -------------------------------------------------------
    abrirModalAsignacion(itemId, nombreItem) {
        const elId = document.getElementById('panol-item-id');
        const elNombre = document.getElementById('panol-item-nombre');
        const selectTec = document.getElementById('panol-tecnico');
        const elFecha = document.getElementById('panol-fecha-entrega');
        const elObs = document.getElementById('panol-observacion');
        const elCant = document.getElementById('panol-cantidad');
        const modal = document.getElementById('modal-asignacion-panol');
        ...
        elId.value = itemId;
        if (elNombre) elNombre.textContent = nombreItem;

        if (selectTec && appData?.empleados) {
            ...
            appData.empleados
                ...
                .forEach(e => {
                    const opt = document.createElement('option');
                    opt.value = e.id;
                    opt.textContent = `${e.nombre1} ${e.apepaterno}`;
    ...
    // -------------------------------------------------------
    // 5. CONFIRMAR ASIGNACIÓN (corregida: stock atómico)
    // -------------------------------------------------------
    async confirmarAsignacion(tecnicoId, itemId, cantidad, observacion) {
        console.log('confirmarAsignacion llamada con:', { tecnicoId, itemId, cantidad, observacion });
        if (!tecnicoId || !itemId || !cantidad || cantidad <= 0) {
            mostrarToast('Datos de asignación incompletos', 'error');
            return false;
        }

        mostrarLoader('Asignando material...');
        try {
            // 1. Obtener datos actuales del ítem
            const { data: item, error: errItem } = await supabase
                .from('panol_items')
                .select('stock_total, stock_asignado, nombre')
                .eq('id', itemId)
                .single();
            if (errItem || !item) throw new Error('Artículo no encontrado');

            const disponibleActual = item.stock_total - (item.stock_asignado || 0);
            if (disponibleActual < cantidad) {
                throw new Error(`Stock insuficiente. Disponible: ${disponibleActual}, solicitado: ${cantidad}`);
            }

            // 2. Crear registro de asignación
            const { error: errAsig } = await supabase
                .from('panol_asignaciones')
                .insert([{
            ...
            // 3. Actualizar stock_asignado (NO stock_disponible)
            const nuevoAsignado = (item.stock_asignado || 0) + cantidad;
            await supabase
                .from('panol_items')
                .update({
                    stock_asignado: nuevoAsignado,
                    estado: (item.stock_total - nuevoAsignado) > 0 ? 'disponible' : 'agotado'
                })
                .eq('id', itemId);

            mostrarToast(`✅ Asignación exitosa: ${cantidad} × "${item.nombre}"`, 'success');
            return true;
    ...
    // -------------------------------------------------------
    // 6. ABRIR MODAL AGREGAR STOCK
    // -------------------------------------------------------
    abrirModalStock(categoria) {
        const modal = document.getElementById('modal-agregar-stock-panol');
        if (!modal) { console.error('❌ Modal de stock no encontrado'); return; }

        const tituloMap = { herramientas: 'Herramientas', epp: 'EPP', uniformes: 'Uniformes' };
        const titulo = document.getElementById('modal-stock-titulo');
        if (titulo) titulo.innerHTML = `➕ Agregar Stock — ${tituloMap[categoria] || categoria}`;

        const hiddenCat = document.getElementById('stock-categoria');
        if (hiddenCat) hiddenCat.value = categoria;

        const esUniforme = categoria === 'uniformes';
        const esEpp = categoria === 'epp';
        const grupoTalla = document.getElementById('grupo-stock-talla');
        const inputTalla = document.getElementById('stock-talla');

        if (grupoTalla) grupoTalla.style.display = (esUniforme || esEpp) ? 'block' : 'none';
        if (inputTalla) {
            inputTalla.value = '';
            if (esUniforme) inputTalla.placeholder = 'Ej: S, M, L, XL, XXL';
            else if (esEpp) inputTalla.placeholder = 'Ej: Estándar, Ajustable';
        }

        ['stock-nombre','stock-codigo'].forEach(id => {
            const el = document.getElementById(id);
            if (el) el.value = '';
        });
        const cant = document.getElementById('stock-cantidad');
        if (cant) cant.value = '1';
        const ubic = document.getElementById('stock-ubicacion');
        if (ubic) ubic.value = 'Bodega Principal';
        const umbral = document.getElementById('stock-umbral');
        if (umbral) umbral.value = '5';

        // Fecha vencimiento (solo EPP)
        const grupoVenc = document.getElementById('grupo-stock-vencimiento');
        if (grupoVenc) grupoVenc.style.display = esEpp ? 'block' : 'none';

        modal.style.display = 'flex';
    },

    // -------------------------------------------------------
    // 7. CERRAR MODAL STOCK
    // -------------------------------------------------------
    cerrarModalStockPanol() {
        const modal = document.getElementById('modal-agregar-stock-panol');
        if (modal) modal.style.display = 'none';
    },

    // -------------------------------------------------------
    // 7c. ABRIR DIÁLOGO AJUSTE STOCK (inline, sin modal extra)
    // -------------------------------------------------------
    abrirAjusteStock(itemId, nombreItem, stockActual) {
        const nuevoVal = prompt(`Ajustar stock total de:\n"${nombreItem}"\n\nStock actual: ${stockActual}\nNuevo stock total:`, stockActual);
        if (nuevoVal === null) return; // canceló
        this.ajustarStock(itemId, nuevoVal);
    },

    // -------------------------------------------------------
    // 7b. AJUSTAR STOCK DE UN ITEM EXISTENTE
    // -------------------------------------------------------
    async ajustarStock(itemId, nuevoTotal) {
        if (!itemId || nuevoTotal === undefined || nuevoTotal === null) return;
        if (isNaN(parseInt(nuevoTotal)) || parseInt(nuevoTotal) < 0) {
            return mostrarToast('Ingrese un valor de stock válido (0 o mayor)', 'error');
        }
        const total = parseInt(nuevoTotal);
        mostrarLoader('Ajustando stock...');
        try {
            // Obtener asignado actual (solo para mostrar en el mensaje final)
            const { data: item, error: errGet } = await supabase
                .from('panol_items')
                .select('stock_asignado, nombre')
                .eq('id', itemId)
                .single();
            if (errGet || !item) throw new Error('No se pudo obtener el artículo');

            const asignado = item.stock_asignado || 0;
            // El disponible se recalculará automáticamente en la BD: stock_total - stock_asignado
            // Por eso NO actualizamos stock_disponible aquí

            const { error: errUpd } = await supabase
                .from('panol_items')
                .update({
                    stock_total: total,
                    estado: total > asignado ? 'disponible' : 'agotado'
                })
                .eq('id', itemId);
            if (errUpd) throw errUpd;
            // Mostrar el nuevo disponible (lo podemos consultar de nuevo o calcular localmente)
            const nuevoDisponible = Math.max(0, total - asignado);
            mostrarToast(`✅ Stock ajustado: "${item.nombre}" → ${nuevoDisponible} disponibles (${asignado} asignados)`, 'success');

            // Refrescar tabla activa
            // Refrescar tabla activa
            const panelActivo = document.querySelector('[id^="panel-panol-"]');
            if (panelActivo) {
                const cat = panelActivo.id.replace('panel-panol-', '');
                await this.renderPañol(cat);
            } else {
                // fallback: recargar los tres
                await this.renderPañol('herramientas');
            }
        } catch (err) {
    ...
    // -------------------------------------------------------
    // 8. GUARDAR STOCK (corregida: verifica categoría + código único)
    // -------------------------------------------------------
    async guardarStockPanol() {
        ...
        const ubicacion = document.getElementById('stock-ubicacion')?.value.trim() || 'Bodega Principal';
        const umbral = parseInt(document.getElementById('stock-umbral')?.value || '5');
        const talla = document.getElementById('stock-talla')?.value.trim() || null;
        const fechaVencimiento = document.getElementById('stock-fecha-vencimiento')?.value || null;

        if (!nombre) return mostrarToast('Ingrese el nombre del artículo', 'error');
        if (!codigo) return mostrarToast('Ingrese el código/serie', 'error');
        if (!cantidad || cantidad < 1) return mostrarToast('Ingrese una cantidad válida (mínimo 1)', 'error');

        mostrarLoader('Guardando stock...');

        try {
            const tipoMap = { herramientas: 'herramienta', epp: 'epp', uniformes: 'uniforme' };
            const tipo = tipoMap[categoria];
            if (!tipo) throw new Error(`Categoría "${categoria}" no válida`);
            ...
            if (catError || !catData) throw new Error(`Categoría "${categoria}" no encontrada en BD. Ejecuta el script de datos iniciales.`);

            // Verificar código único
            const { data: existente } = await supabase
                .from('panol_items')
                .select('id')
                .eq('codigo_serie', codigo)
                .maybeSingle();

            if (existente) throw new Error(`Ya existe un artículo con el código "${codigo}"`);

            // Insertar nuevo item (sin stock_disponible)
            ...
            mostrarToast(`✅ ${cantidad} × "${nombre}" agregado al inventario`, 'success');
            this.cerrarModalStockPanol();
            await this.renderPañol(categoria);

        } catch (err) {
            console.error('❌ Error al guardar stock pañol:', err);
            mostrarToast('❌ ' + err.message, 'error');
        } finally {
            ocultarLoader();
        }
    ...
    // -------------------------------------------------------
    // 9. VER EXISTENCIA POR TÉCNICO
    // Consulta panol_asignaciones activas de un técnico
    // -------------------------------------------------------
    async cargarExistenciaTecnico(tecnicoId) {
        const contenedor = document.getElementById('panol-existencia-resultado');
        if (!contenedor) return;

        if (!tecnicoId) {
            contenedor.innerHTML = '<p style="color:#888;text-align:center;padding:20px;">Seleccione un técnico para ver sus artículos asignados.</p>';
            return;
        }

        contenedor.innerHTML = '<p style="text-align:center;color:#888;padding:20px;">Cargando...</p>';

        try {
            const { data, error } = await supabase
                .from('panol_asignaciones')
                .select(`
                    id, cantidad, fecha_entrega, observacion, registrado_por,
                    item:panol_items(
                        nombre, codigo_serie, talla_modelo,
                        categoria:panol_categorias(nombre, tipo)
                    )
                `)
            ...
            if (!data || data.length === 0) {
                contenedor.innerHTML = '<p style="text-align:center;color:#888;padding:30px;">Este técnico no tiene artículos asignados actualmente.</p>';
                return;
            }
            ...
            const grupos = {};
            data.forEach(asig => {
                const cat = asig.item?.categoria?.nombre || 'Sin categoría';
                if (!grupos[cat]) grupos[cat] = [];
                grupos[cat].push(asig);
            });

            const totalItems = data.reduce((sum, a) => sum + (a.cantidad || 1), 0);

            let html = `
                <div style="margin-bottom:16px;padding:12px 16px;background:#e8f4fd;border-radius:8px;border-left:4px solid #007bff;">
                    <strong style="color:#0056b3;">Total asignado: ${totalItems} artículo(s) en ${data.length} registro(s)</strong>
                </div>`;

            Object.entries(grupos).forEach(([catNombre, items]) => {
                const iconos = { 'Herramientas': '🔨', 'EPP': '🦺', 'Uniformes': '👕' };
                const icono = iconos[catNombre] || '📦';
                html += `
                <div style="margin-bottom:20px;">
                    <h4 style="margin:0 0 10px;color:#374151;font-size:14px;font-weight:600;border-bottom:1px solid #fcfdfd;padding-bottom:6px;">
                        ${icono} ${catNombre} (${items.length})
                    </h4>
                    <table style="width:100%;border-collapse:collapse;font-size:13px;">
                        <thead>
                            <tr style="background:white;">
                                <th style="padding:8px 10px;text-align:left;color:#6b7280;font-weight:500;">Artículo</th>
                                <th style="padding:8px 10px;text-align:left;color:#6b7280;font-weight:500;">Código</th>
                                <th style="padding:8px 10px;text-align:center;color:#6b7280;font-weight:500;">Cant.</th>
                                <th style="padding:8px 10px;text-align:left;color:#6b7280;font-weight:500;">Fecha entrega</th>
                                <th style="padding:8px 10px;text-align:center;color:#6b7280;font-weight:500;">Devolución</th>
                            </tr>
                        </thead>
                        <tbody>`;

                items.forEach(asig => {
                    const nombre = asig.item?.nombre || '—';
                    const talla = asig.item?.talla_modelo ? ` (${asig.item.talla_modelo})` : '';
                    const codigo = asig.item?.codigo_serie || '—';
                    const cantidad = asig.cantidad || 1;
                    const fecha = asig.fecha_entrega
                        ? new Date(asig.fecha_entrega + 'T00:00:00').toLocaleDateString('es-CL')
                        : '—';

                    html += `
                        <tr style="border-bottom:1px solid #f3f4f6;">
                            <td style="padding:8px 10px;color:#111827;">${nombre}${talla}</td>
                            <td style="padding:8px 10px;font-family:monospace;color:#0056b3;font-size:12px;">${codigo}</td>
                            <td style="padding:8px 10px;text-align:center;font-weight:600;">${cantidad}</td>
                            <td style="padding:8px 10px;color:#6b7280;">${fecha}</td>
                            <td style="padding:8px 10px;text-align:center;">
                                <button onclick="window.PanolModule.registrarDevolucionItem('${asig.id}','${nombre.replace(/'/g,"\'")}',${cantidad})"
                                    style="padding:4px 10px;background:#fff;color:#dc3545;border:1px solid #dc3545;border-radius:5px;cursor:pointer;font-size:12px;">
                                    ↩️ Devolver
                                </button>
                            </td>
                        </tr>`;
            ...
            console.error('❌ Error cargando existencia técnico:', err);
            contenedor.innerHTML = `<p style="color:#dc3545;padding:20px;">❌ Error: ${err.message}</p>`;
        }
    },

    // -------------------------------------------------------
    // 9b. REGISTRAR DEVOLUCIÓN DE UN ITEM DESDE EXISTENCIA TÉCNICO
    // -------------------------------------------------------
    async registrarDevolucionItem(asignacionId, nombreItem, cantidadAsignada) {
        const motivo = prompt(`Devolver: "${nombreItem}"\n\nMotivo de devolución (opcional):`);
        if (motivo === null) return; // canceló

        mostrarLoader('Registrando devolución...');
        try {
            // 1. Obtener la asignación para saber item_id y cantidad
            const { data: asig, error: errGet } = await supabase
                .from('panol_asignaciones')
                .select('item_id, cantidad')
                .eq('id', asignacionId)
                .single();
            if (errGet || !asig) throw new Error('No se encontró la asignación');

            // 2. Marcar asignación como devuelta
            const { error: errUpd } = await supabase
                .from('panol_asignaciones')
                .update({
                    estado: 'devuelto',
                    fecha_devolucion: new Date().toISOString().split('T')[0],
                    motivo_devolucion: motivo || null
                })
                .eq('id', asignacionId);
            if (errUpd) throw errUpd;

            // 3. Actualizar stock del item (solo stock_asignado)
            const { data: item, error: errItem } = await supabase
                .from('panol_items')
                .select('stock_asignado, stock_total')
                .eq('id', asig.item_id)
                .single();

            if (item) {
                const nuevoAsignado = Math.max(0, (item.stock_asignado || 0) - (asig.cantidad || 1));
                // No actualizamos stock_disponible, la BD lo hará automáticamente
                await supabase
                    .from('panol_items')
                    .update({
                        stock_asignado: nuevoAsignado,
            ...
            mostrarToast(`✅ Devolución registrada: "${nombreItem}"`, 'success');

            // Refrescar la vista de existencia
            const selectTec = document.getElementById('panol-existencia-tecnico');
            if (selectTec?.value) {
                await this.cargarExistenciaTecnico(selectTec.value);
            }
    ...
    // -------------------------------------------------------
    // 9c. INICIALIZAR PANEL EXISTENCIA TÉCNICO
    // -------------------------------------------------------
    inicializarPanelExistencia() {
        const selectTec = document.getElementById('panol-existencia-tecnico');
        if (!selectTec) return;

        // Llenar select con empleados activos
        selectTec.innerHTML = '<option value="">-- Seleccione Técnico --</option>';
        if (appData?.empleados) {
            appData.empleados
                .filter(e => e.activo)
                .sort((a, b) => `${a.nombre1} ${a.apepaterno}`.localeCompare(`${b.nombre1} ${b.apepaterno}`))
                .forEach(e => {
        ...
        // Listener del select (usar onchange para evitar duplicados)
        selectTec.onchange = () => this.cargarExistenciaTecnico(selectTec.value);
    },

    // -------------------------------------------------------
    // 10. IMPRIMIR GUÍA DE ENTREGA
    // -------------------------------------------------------
    imprimirGuia(itemNombre, tecnicoNombre, cantidad, fecha) {
        const guia = `PANOL-${new Date().toISOString().slice(0,10).replace(/-/g,'')}-${Math.random().toString(36).slice(-4).toUpperCase()}`;
        const contenido = `
        <html><body style="font-family:Arial,sans-serif;padding:40px;max-width:600px;margin:auto;">
            <h2 style="border-bottom:2px solid #003366;padding-bottom:10px;color:#003366;">GUÍA DE ENTREGA — PAÑOL</h2>
            <table style="width:100%;border-collapse:collapse;margin-top:20px;">
                <tr><td style="padding:8px;color:#555;width:40%;">N° Guía:</td><td style="padding:8px;font-weight:bold;">${guia}</td></tr>
                <tr style="background:#f8f9fa;"><td style="padding:8px;color:#555;">Fecha:</td><td style="padding:8px;">${fecha || new Date().toLocaleDateString('es-CL')}</td></tr>
                <tr><td style="padding:8px;color:#555;">Técnico:</td><td style="padding:8px;">${tecnicoNombre || '—'}</td></tr>
                <tr style="background:#f8f9fa;"><td style="padding:8px;color:#555;">Artículo:</td><td style="padding:8px;">${itemNombre || '—'}</td></tr>
                <tr><td style="padding:8px;color:#555;">Cantidad:</td><td style="padding:8px;font-weight:bold;">${cantidad}</td></tr>
            </table>
            <div style="margin-top:60px;display:grid;grid-template-columns:1fr 1fr;gap:40px;">
                <div style="border-top:1px solid #333;padding-top:8px;text-align:center;color:#555;">Firma Técnico</div>
                <div style="border-top:1px solid #333;padding-top:8px;text-align:center;color:#555;">Firma Pañolero</div>
            </div>
        </body></html>`;
        const ventana = window.open('', '_blank', 'width=650,height:500');
        if (ventana) { ventana.document.write(contenido); ventana.document.close(); ventana.print(); }
    }
};

// =======================================================
// === EXPOSICIÓN GLOBAL DEL MÓDULO PAÑOL ===
// =======================================================
window.PanolModule = PanolModule;

// Alias globales para compatibilidad con llamadas inline del HTML
window.confirmarAsignacionPañol = async () => {
    const tecnicoId  = document.getElementById('panol-tecnico')?.value?.trim();
    const itemId     = document.getElementById('panol-item-id')?.value?.trim();
    const cantidad   = parseInt(document.getElementById('panol-cantidad')?.value || '0');
    const observacion = document.getElementById('panol-observacion')?.value?.trim() || '';

    const ok = await window.PanolModule.confirmarAsignacion(tecnicoId, itemId, cantidad, observacion);
    if (ok) {
        // Cerrar modal si la asignación fue exitosa
        const modal = document.getElementById('modal-asignacion-panol');
        if (modal) modal.style.display = 'none';
        // Refrescar tabla
        const panelActivo = document.querySelector('[id^="panel-panol-"]');
        if (panelActivo) {
            const cat = panelActivo.id.replace('panel-panol-', '');
            await window.PanolModule.renderPañol(cat);
        }
    }
};
window.abrirModalPañol           = (cat) => window.PanolModule.abrirModalStock(cat);
window.cerrarModalStockPanol     = () => window.PanolModule.cerrarModalStockPanol();
window.guardarStockPanol         = () => window.PanolModule.guardarStockPanol();
window.renderPañol               = (cat) => window.PanolModule.renderPañol(cat);
...
// [IIFE initModuloEquipos eliminado - lógica ya manejada por el listener orden-sub en DOMContentLoaded]
